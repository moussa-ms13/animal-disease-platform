...
export function KpiCard({ label, value, detail, icon: Icon = Info, tone = 'navy', trend }) {
  return <article className={cx('kpi-card', `kpi-${tone}`)}><div className="kpi-icon"><Icon size={18} /></div><div className="min-w-0"><p className="kpi-label">{label}</p><p className="kpi-value">{value}</p>{detail && <p className="kpi-detail">{detail}</p>}{trend && <p className="kpi-trend">{trend}</p>}</div></article>;
}
